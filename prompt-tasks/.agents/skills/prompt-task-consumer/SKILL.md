...
- Running `prompt-tasks/.agents/loop/consumer.md`
...
- one chosen `prompt-tasks/tasks/PT-*.md` task card
...
- If the queue has no actionable `TODO`, append a note to `prompt-tasks/tasks/notes.md` and stop.

### 2. Claim it visibly

- Move the task card to `Status: DOING`.
- Update the matching line in `prompt-tasks/tasks/index.md`.
